...
class ExtractCharactersRequest(BaseModel):
    ...
    screenplay_json_path: str = Field(
        ..., description="Path to screenplay JSON file"
    )
...
class GenerateYamlRequest(BaseModel):
    ...
    include_comments: bool = Field(default=True, description="Include character stats and notes as comments")
...
async def extract_characters(request: ExtractCharactersRequest) -> ExtractCharactersResponse:
    """Extract characters from screenplay JSON file with their statistics."""
    try:
        return await voice_casting_service.extract_characters(request.screenplay_json_path)
    except Exception as e:
        raise HTTPException(
            status_code=500,
            detail=f"Failed to extract characters: {str(e)}"
        )
...
class GenerateCharacterNotesPromptRequest(BaseModel):
    ...
    custom_prompt_path: Optional[str] = Field(None, description="Optional custom prompt file path")


class GenerateCharacterNotesPromptResponse(BaseModel):
    """Response with generated character notes prompt."""
    
    prompt_content: str = Field(..., description="Generated prompt text for LLM")
    privacy_notice: str = Field(..., description="Privacy notice text to display to user")


class GenerateVoiceLibraryPromptRequest(BaseModel):
    """Request to generate voice library casting prompt for LLM."""
    
    yaml_content: str = Field(..., description="Voice configuration YAML with character notes")
    providers: List[str] = Field(..., description="List of providers to include in prompt")
    custom_prompt_path: Optional[str] = Field(None, description="Optional custom prompt file path")
...
@router.post("/generate-character-notes-prompt", response_model=GenerateCharacterNotesPromptResponse)
async def generate_character_notes_prompt(request: GenerateCharacterNotesPromptRequest) -> GenerateCharacterNotesPromptResponse:
...
@router.post("/generate-voice-library-prompt", response_model=GenerateVoiceLibraryPromptResponse)
async def generate_voice_library_prompt(request: GenerateVoiceLibraryPromptRequest) -> GenerateVoiceLibraryPromptResponse:
    ...
    try:
        ...
        raise HTTPException(
            status_code=500,
            detail=f"Failed to generate voice library prompt: {str(e)}"
        )
...
async def upload_json(file: UploadFile = File(...)) -> VoiceCastingSession:
    ...
    try:
        ...
        raise HTTPException(
            status_code=500,
            detail=f"Failed to create voice casting session: {str(e)}"
        )
...
async def create_session_from_task(request: CreateSessionFromTaskRequest) -> VoiceCastingSession:
    """
    ...
    if not parsing_result:
        raise HTTPException(
            status_code=404,
            detail=f"Task {request.task_id} not found or not completed"
        )
...
async def get_session(session_id: str) -> VoiceCastingSession:
    ...
    if not session:
        raise HTTPException(
            status_code=404,
            detail=f"Voice casting session {session_id} not found"
        )
    
    return session


@router.post("/session/{session_id}/screenplay-source", response_model=VoiceCastingSession)
async def upload_screenplay_source(
    ...
    if file.filename.endswith('.txt'):
        try:
            ...
            raise HTTPException(
                status_code=400,
                detail="TXT file must be valid UTF-8 encoded text"
            )
