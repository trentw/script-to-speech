...
from script_to_speech.voice_casting.character_notes_utils import generate_voice_casting_prompt_file
from script_to_speech.voice_casting.voice_library_casting_utils import generate_voice_library_casting_prompt_file
...
class VoiceCastingService:
    ...
    async def extract_characters(self, screenplay_json_path: str) -> ExtractCharactersResponse:
        """
        ...
        try:
            safe_path = self._path_validator.validate_existing_path(Path(screenplay_json_path))
    ...
    async def validate_yaml(
        ...
        for speaker, config in yaml_data.items():
            ...
            try:
                provider_class = self._tts_manager._get_provider_class(provider_name)
                ...
                if "sts_id" in validation_config:
                    sts_id = validation_config.pop("sts_id")  # Remove sts_id from config
                    
                    try:
                        # Get expansion from voice library
                        expanded_config = self._voice_library.expand_config(provider_name, sts_id)
                        ...
                        invalid_configs[speaker] = f"Invalid sts_id '{sts_id}': {str(ve)}"
                        continue
                    except Exception as ve:
                        invalid_configs[speaker] = f"Failed to expand sts_id '{sts_id}': {str(ve)}"
                        continue
    ...
    async def generate_yaml(
        ...
        for idx, assignment in enumerate(sorted_assignments):
            ...
            if include_comments:
                comment_lines = self._build_character_comments(assignment, char_info_map)
                if comment_lines:
                    ...
                    if idx == 0:
                        ...
                        all_comments = [""] + comment_lines  # Add empty line before comments
    ...
    def _sort_assignments(self, assignments: List[VoiceAssignment], character_info: List[CharacterInfo]) -> List[VoiceAssignment]:
        ...
        return sorted(
            ...
            key=lambda a: (
                a.character != "default",
                -(a.line_count or char_info_map.get(a.character, CharacterInfo(name=a.character, line_count=0, total_characters=0, longest_dialogue=0)).line_count),
                a.character
            )
        )
    
    def _build_character_comments(self, assignment: VoiceAssignment, char_info_map: Dict[str, CharacterInfo]) -> List[str]:
        ...
        total_chars = assignment.total_characters or (char_info.total_characters if char_info else 0)
        longest_dialogue = assignment.longest_dialogue or (char_info.longest_dialogue if char_info else 0)
        
        if line_count > 0:
            comments.append(f"{assignment.character}: {line_count} lines")
            comments.append(f"Total characters: {total_chars}, Longest dialogue: {longest_dialogue} characters")
    ...
    async def parse_yaml(self, yaml_content: str) -> ParseYamlResponse:
        ...
        try:
            ...
            if not isinstance(yaml_data, dict):
                errors.append("YAML must be a mapping of speakers to voice configurations")
                return ParseYamlResponse(
                    assignments=[],
                    has_errors=True,
                    errors=errors
            ...
            for speaker, config in yaml_data.items():
                ...
                if not isinstance(config, dict):
                    errors.append(f"{speaker}: Configuration must be a dictionary, got {type(config).__name__}")
                    continue
                ...
                sts_id = config.get("sts_id") or config.get("voice_id", "")  # Default to empty string
            ...
            duplicate_key_match = re.search(r'found duplicate key "([^"]+)"', error_msg)
            if duplicate_key_match:
                errors.append(f"Duplicate speaker found: {duplicate_key_match.group(1)}")
            else:
            ...
            if "found undefined alias" in str(e):
                ...
                errors.append("YAML contains multiple documents (use single document only)")
            else:
    ...
    def _extract_yaml_comments(self, yaml_content: str) -> Dict[str, Dict[str, Any]]:
        ...
        line_count_pattern = re.compile(YAML_COMMENT_PATTERNS['LINE_COUNT'], re.IGNORECASE)
        char_stats_pattern = re.compile(YAML_COMMENT_PATTERNS['CHAR_STATS'], re.IGNORECASE)
        casting_notes_pattern = re.compile(YAML_COMMENT_PATTERNS['CASTING_NOTES'], re.IGNORECASE)
            ...
            if stripped and not stripped.startswith('#') and stripped.endswith(':'):
                ...
                if not line.startswith(' '):
                    ...
                    if comment_buffer:
                        ...
                        if casting_notes or roles or additional_notes or line_count is not None:
                            character_comments[char_name] = {}
                            if casting_notes:
                                # Join multi-line casting notes with spaces
                                character_comments[char_name]['casting_notes'] = ' '.join(casting_notes)
                            if roles:
                                # Join multi-line roles with spaces
                                character_comments[char_name]['role'] = ' '.join(roles)
                            if additional_notes:
                                character_comments[char_name]['additional_notes'] = additional_notes
                            if line_count is not None:
                                character_comments[char_name]['line_count'] = line_count
                            if total_characters is not None:
                                character_comments[char_name]['total_characters'] = total_characters
                            if longest_dialogue is not None:
                                character_comments[char_name]['longest_dialogue'] = longest_dialogue
    ...
    async def generate_character_notes_prompt(
        ...
        if not session.screenplay_source_path:
            raise ValueError(f"Session {session_id} does not have a screenplay source path. Please upload the original PDF/TXT file.")
    ...
    async def create_session_from_task(self, parsing_result: Dict[str, Any]) -> VoiceCastingSession:
        """
        ...
        logger.info(f"Created voice casting session {session.session_id} from parsing task")
        return session
    ...
    async def update_session(self, session_id: str, status: str) -> Optional[VoiceCastingSession]:
        """
