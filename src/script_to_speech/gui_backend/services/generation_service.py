...
class GenerationService:
    ...
    async def _process_generation_task(self, task: GenerationTask) -> None:
        ...
        try:
            ...
            task.result = GenerationResult(
                ...
                text_preview=(
                    task.request.text[:50] + "..."
                    if len(task.request.text) > 50
                    else task.request.text
                ),
    ...
    async def _generate_audio_files(
        self, task: GenerationTask, tts_manager: TTSProviderManager
    ) -> List[str]:
    ...
    def _find_generated_file(
        self, request: GenerationRequest, variant: int, tts_manager: TTSProviderManager
    ) -> Optional[str]:
        ...
        try:
            ...
            recent_files = [
                f
                for f in settings.AUDIO_OUTPUT_DIR.glob("*.mp3")
