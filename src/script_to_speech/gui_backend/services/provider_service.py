...
class ProviderService:
    ...
    def _load_providers(self) -> None:
        ...
        try:
            ...
            real_providers = [p for p in available_providers if not p.startswith('dummy_')]
            
            for provider_name in real_providers:
                try:
                    provider_class = get_provider_class(provider_name)
                    provider_info = self._extract_provider_info(provider_name, provider_class)
    ...
    def validate_config(self, provider_name: str, config: Dict[str, Any]) -> ValidationResult:
        ...
        try:
            ...
            all_fields = {f.name: f for f in provider_info.required_fields + provider_info.optional_fields}
    ...
    def _validate_field_value(self, field: ProviderField, value: Any) -> str | None:
        ...
        if field.type in (FieldType.INTEGER, FieldType.FLOAT) and isinstance(value, (int, float)):
