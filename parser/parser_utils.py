...
def process_json_chunks(
    ...
    try:
        ...
        log_file = log_dir / f"[process_chunks]_{sanitized_name}_{timestamp}.log"
...
if __name__ == "__main__":
    ...
    process_parser = subparsers.add_parser(
        "process", help="Process screenplay files (PDF or TXT) to generate JSON chunks"
    )
    process_parser.add_argument("input_file", help="Path to input file (PDF or TXT)")
    ...
    analyze_parser = subparsers.add_parser(
        "analyze", help="Analyze an existing screenplay JSON chunks file"
    )
    analyze_parser.add_argument("json_file", help="Path to JSON chunks file")

    # Process JSON command
    process_json_parser = subparsers.add_parser(
        "process-json",
        help="Process existing JSON chunks through preprocessors and processors",
    )
    process_json_parser.add_argument("json_file", help="Path to JSON chunks file")
    ...
    process_json_parser.add_argument(
        "--output-path", help="Custom output path for modified chunks"
    )
