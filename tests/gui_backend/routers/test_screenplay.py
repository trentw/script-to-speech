...
class TestScreenplayUpload:
    ...
    def test_upload_valid_pdf_file(
        self, client: TestClient, mock_screenplay_service, mock_file_system
    ):
        ...
        # Make the mock async
        mock_screenplay_service.create_parsing_task = AsyncMock(
    ...
    def test_upload_with_text_only_flag(
        self, client: TestClient, mock_screenplay_service, mock_file_system
    ):
    ...
    def test_upload_service_error(
        ...
        # Arrange
        mock_screenplay_service.create_parsing_task.side_effect = Exception(
            "Service error"
        )
...
class TestScreenplayTasks:
    ...
    def test_get_all_tasks(
        self, client: TestClient, mock_screenplay_service, sample_task_status
    ):
    ...
    def test_get_task_status_not_found(
        self, client: TestClient, mock_screenplay_service
    ):
    ...
    def test_get_task_status_service_error(
        self, client: TestClient, mock_screenplay_service
    ):
        """Test handling of service errors when getting task status."""
        # Arrange
        task_id = "error-task-id"
        mock_screenplay_service.get_task_status.side_effect = Exception(
            "Database error"
        )
...
class TestScreenplayResults:
    ...
    def test_get_parsing_result_success(
        self, client: TestClient, mock_screenplay_service
    ):
    ...
    def test_get_parsing_result_not_found(
        self, client: TestClient, mock_screenplay_service
    ):
...
class TestScreenplayRecent:
    ...
    def test_get_recent_screenplays_with_limit(
        self, client: TestClient, mock_screenplay_service
    ):
...
class TestScreenplayCleanup:
    ...
    def test_cleanup_old_tasks(self, client: TestClient, mock_screenplay_service):
        ...
        response_data = response.json()
        assert (
            response_data["message"] == f"Cleaned up {removed_count} old parsing tasks"
        )
        mock_screenplay_service.cleanup_old_tasks.assert_called_once_with(24)

    def test_cleanup_old_tasks_custom_age(
        self, client: TestClient, mock_screenplay_service
    ):
        ...
        response = client.delete(
            f"/api/screenplay/cleanup?max_age_hours={max_age_hours}"
        )
...
class TestScreenplayDownload:
    ...
    def test_download_json_file(
        ...
        with patch(
            "script_to_speech.gui_backend.routers.screenplay.FileResponse"
            ...
            mock_file_response.return_value = JSONResponse(
                ...
                headers={
                    "content-disposition": 'attachment; filename="test_screenplay.json"'
                },
            )
            ...
            assert (
                ...
                == 'attachment; filename="test_screenplay.json"'
            )
    ...
    def test_download_text_file(
        ...
        with patch(
            ...
            mock_file_response.return_value = JSONResponse(
                content={"message": "file download"},
                headers={
                    "content-disposition": 'attachment; filename="test_screenplay.txt"'
                },
            )
            ...
            assert (
                response.headers["content-disposition"]
                == 'attachment; filename="test_screenplay.txt"'
            )
            # Verify FileResponse was called with correct parameters
            mock_file_response.assert_called_once_with(
    ...
    def test_download_file_task_not_found(
        self, client: TestClient, mock_screenplay_service
    ):
    ...
    def test_download_file_not_exists(
        self, mock_exists, client: TestClient, mock_screenplay_service
    ):
