...
class TestDualDialogPreProcessor:
    """Tests for the DualDialogPreProcessor class."""

    def test_validate_config_empty(self):
        """Test validation with empty configuration."""
        # Arrange
        config = {}
        processor = DualDialogPreProcessor(config)
        ...
        assert (
            ...
            == DualDialogPreProcessor.DEFAULT_MIN_SPEAKER_SPACING
        )
        assert (
            processor.min_dialog_spacing
            == DualDialogPreProcessor.DEFAULT_MIN_DIALOG_SPACING
        )

    def test_validate_config_valid(self):
        """Test validation with valid configuration."""
        # Arrange
        config = {"min_speaker_spacing": 8, "min_dialog_spacing": 4}
        processor = DualDialogPreProcessor(config)
        ...
        assert processor.min_dialog_spacing == 4
    ...
    def test_validate_config_invalid_type(self):
        ...
        # Arrange - Test with string for dialog spacing
        config = {"min_speaker_spacing": 8, "min_dialog_spacing": "4"}
        processor = DualDialogPreProcessor(config)
    ...
    def test_validate_config_invalid_value(self):
        ...
        config = {"min_speaker_spacing": 0, "min_dialog_spacing": 4}
        processor = DualDialogPreProcessor(config)

        # Act & Assert
        assert processor.validate_config() is False

        # Arrange - Test with negative dialog spacing
        config = {"min_speaker_spacing": 8, "min_dialog_spacing": -1}
        processor = DualDialogPreProcessor(config)
    ...
    def test_process_empty_list(self):
        ...
        # Arrange
        processor = DualDialogPreProcessor({})
    ...
    def test_split_speakers_full_custom_spacing(self):
        ...
        processor = DualDialogPreProcessor({"min_speaker_spacing": 10})
    ...
    def test_process_basic_dual_dialog(self):
        """Test basic processing of a dual dialog chunk pair."""
        # Arrange
        processor = DualDialogPreProcessor({})
        chunks = [
            {
        ...
        # Second chunk should be dialog for left speaker
        ...
        # Fourth chunk should be dialog for right speaker
        assert result[3]["type"] == "dialog"
        assert result[3]["text"] == "Woody ain't singing anymore, is he?"
        assert result[3]["speaker"] == "FOLK MUSICIAN # 2"

    def test_process_dual_dialog_with_midcolumn_speaker_change(self):
        """Test processing of dual dialog with new speaker introduced in the middle of a column."""
        # Arrange
        processor = DualDialogPreProcessor({})
        chunks = [
            {
        ...
        # Left column: MCKAY -> parenthetical -> dialog
        assert result[0]["type"] == "speaker_attribution"
        ...
        # Right column: VTR TECH -> parenthetical -> dialog -> GEOFF -> parenthetical -> dialog
        assert result[3]["type"] == "speaker_attribution"
        assert result[3]["text"] == "VTR TECH"
        assert result[4]["type"] == "dialog_modifier"
        assert result[4]["text"] == "(quietly into headset)"
        assert result[5]["type"] == "dialog"
        assert (
            result[5]["text"]
        ...
        assert result[7]["type"] == "dialog_modifier"
        assert result[7]["text"] == "(over headset)"
        assert result[8]["type"] == "dialog"
        assert result[8]["text"] == "Hit it."
        assert result[8]["speaker"] == "GEOFF"

    def test_process_dual_dialog_with_language_markers(self):
        """Test processing of dual dialog with language markers in parentheticals."""
        # Arrange
        processor = DualDialogPreProcessor(
            {"min_speaker_spacing": 2, "min_dialog_spacing": 2}
        )
        chunks = [
            {
                "type": "dual_speaker_attribution",
        ...
        # Should have 2 speakers, 2 parentheticals, 2 dialog chunks
        assert len(result) == 6
    ...
    def test_process_noncontiguous_dual_dialog(self):
        """Test that dual dialog processing only happens for contiguous dual_speaker_attribution and dual_dialog."""
        # Arrange
        processor = DualDialogPreProcessor({})
        chunks = [
            {
            ...
            {
                "type": "action",  # Should prevent dual dialog processing
    ...
    def test_process_dialog_splitting_with_custom_spacing(self):
        """Test that dialog splitting uses the configured min_dialog_spacing correctly."""
        # Arrange
        processor = DualDialogPreProcessor({"min_dialog_spacing": 2})
        chunks = [
            {
                "type": "dual_speaker_attribution",
                "speaker": "",
    ...
    def test_process_dual_dialog_with_non_dual_chunks(self):
        """Test processing when chunks include both dual and non-dual dialog elements."""
        # Arrange
        processor = DualDialogPreProcessor({})
        chunks = [
        ...
        assert (
            len(result) == 8
        )  # Original 6 plus 2 new ones (from dual dialog split into 4)
    ...
    def test_split_dialog_indentation_handling(self):
        """Test that dialog splitting properly handles indentation between columns."""
        # Arrange
        processor = DualDialogPreProcessor({})
        raw_dialog = """               First line left               First line right
        ...
        # Act
        left_lines, right_lines = processor._split_dialog(raw_dialog)
    ...
    def test_complex_multilevel_dialog(self):
        """Test processing of complex multilevel dual dialog with nested speakers."""
        # Arrange
        processor = DualDialogPreProcessor({})
        chunks = [
            {
        ...
        # Process once and verify results (already tested in test_process_dual_dialog_with_midcolumn_speaker_change)
    ...
    def test_invalid_split_handling(self):
        ...
        # Arrange
        processor = DualDialogPreProcessor({})
        # Create a dual dialog pair with problematic format - speaker line that looks like it should split
        # but dialog doesn't have clear columns
        chunks = [
            {
                "type": "dual_speaker_attribution",
                "speaker": "",
            ...
            },
            {
                "type": "dual_dialog",
                "speaker": "",
                # Dialog without clear column separation
                "raw_text": "               This dialog doesn't have proper spacing to split into columns      \n               because it's all on the left side without proper format             ",
                "text": "This dialog doesn't have proper spacing to split into columns because it's all on the left side without proper format",
            },
        ]
        ...
        # The first speaker should have all the dialog text
        found_speaker_one_dialog = False
        for chunk in result:
            if chunk["type"] == "dialog" and chunk["speaker"] == "SPEAKER ONE":
                found_speaker_one_dialog = True
                assert "dialog doesn't have proper spacing" in chunk["text"]
                break

        assert (
            found_speaker_one_dialog
        ), "Speaker one should have received the dialog text"
