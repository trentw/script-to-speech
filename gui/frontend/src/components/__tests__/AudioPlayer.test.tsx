...
describe('AudioPlayer', () => {
  ...
  describe('Empty State', () => {
    it('should show empty state when no tasks', () => {
      ...
      expect(
        screen.getByText('Generate speech to see audio player')
      ).toBeInTheDocument();
    });
  ...
  describe('Audio Track Display', () => {
    ...
    it('should truncate long text', () => {
      // Arrange
      const longText =
        'This is a very long text that should be truncated after 60 characters to prevent UI overflow';
      const tasks = [
        createTask({
          request: {
            text: longText,
  ...
  describe('Download Functionality', () => {
    it('should show download button for each track', () => {
      ...
      expect(downloadButtons[1]).toHaveAttribute(
        'data-filename',
        'task-2-0.mp3'
      );
    });
  ...
  describe('UI States', () => {
    it('should highlight current playing track', async () => {
      ...
      const firstTrackContainer = screen
        .getByText('alloy')
        .closest('.rounded-lg');
      const secondTrackContainer = screen
        .getByText('rachel')
        .closest('.rounded-lg');
    ...
    it('should show pause icon for playing track', async () => {
      ...
      await waitFor(() => {
        const pauseButton = screen
          .getAllByRole('button')
          .find((btn) => btn.querySelector('svg.lucide-pause'));
