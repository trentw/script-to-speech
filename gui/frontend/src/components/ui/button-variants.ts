  ...
  "inline-flex items-center justify-center gap-2 whitespace-nowrap text-sm font-medium transition-all duration-200 disabled:pointer-events-none disabled:opacity-50 [&_svg]:pointer-events-none [&_svg:not([class*='size-'])]:size-4 [&_svg]:shrink-0 outline-none focus-visible:ring-2 focus-visible:ring-ring focus-visible:ring-offset-2 cursor-pointer select-none",
  {
    variants: {
      variant: {
        ...
        primary:
          'bg-gray-900 text-white hover:bg-gray-800 hover:shadow-lg active:scale-95 active:shadow-inner shadow-md rounded-md',

        // Secondary action button - standard colors
        secondary:
          'bg-background text-foreground border border-border hover:bg-gray-100 hover:text-foreground hover:border-gray-300 active:scale-95 active:shadow-inner shadow-sm rounded-md',

        // Audio control buttons
        'audio-control':
          'bg-transparent hover:bg-gray-100 hover:text-foreground rounded-full hover:shadow-md active:scale-95 active:shadow-inner',

        // Audio play button (main) - CIRCULAR inverted button for prominence
        'audio-play':
          'bg-gray-900 text-white hover:bg-gray-800 hover:shadow-xl active:scale-95 active:shadow-inner shadow-lg rounded-full border-2 border-gray-900',

        // Action buttons in lists (play, download, more) - more prominent hover
        'list-action':
          'bg-transparent hover:bg-gray-200 hover:text-foreground hover:shadow-md active:scale-95 active:shadow-inner transition-all rounded-md',

        // Reset/secondary buttons
        reset:
          'border border-border bg-background hover:bg-gray-100 hover:text-foreground hover:shadow-md active:scale-95 active:shadow-inner rounded-md',

        // Sidebar toggle
        'sidebar-toggle':
          'hover:bg-gray-100 hover:text-foreground hover:shadow-md active:scale-95 active:shadow-inner rounded-md',

        // Provider/voice selection
        selection:
          'border border-border bg-background hover:bg-gray-100 hover:text-foreground hover:border-gray-300 hover:shadow-md active:scale-95 active:shadow-inner rounded-md',
      },
      size: {
        sm: 'h-8 px-3 text-xs',
