...
export function useRemoveAssignment() {
  ...
  return useMutation<RemoveAssignmentResponse, Error, RemoveAssignmentRequest>({
    mutationFn: async (data) => {
      // Create an empty assignment to "remove" the character
      const emptyAssignment: VoiceAssignment = {
    ...
    onMutate: async (variables) => {
      ...
      if (previousSession) {
        const optimisticSession = produce(previousSession, (draft) => {
          ...
          // The backend will handle removing the assignment from the YAML content
