...
export const useValidateProviderConfig = () => {
  ...
  return useMutation({
    ...
    onError: (error, variables) => {
      console.error(`Configuration validation failed for ${variables.provider}:`, error)
    },
  })
}
...
export const useExpandStsId = () => {
  return useMutation({
    ...
    onError: (error, variables) => {
      console.error(`STS ID expansion failed for ${variables.provider}:${variables.stsId}:`, error)
    },
  })
}