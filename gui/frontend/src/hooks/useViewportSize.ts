...
export function useViewportSize() {
  const { viewportSize, setViewportSize, setSidebarExpanded, setRightPanelExpanded } = useLayout()
  ...
  useEffect(() => {
    function updateViewportSize() {
      ...
      if (newSize !== viewportSize) {
        ...
        if ((newSize === 'mobile' || newSize === 'tablet') && previousSize === 'desktop') {
        ...
        else if (newSize === 'desktop' && (previousSize === 'mobile' || previousSize === 'tablet')) {
          // Stagger sidebar animation to avoid competing with grid layout change
          setTimeout(() => setSidebarExpanded(true), motionTokens.responsiveSidebar.stagger * 1000)
        }
        ...
        else if ((newSize === 'desktop' || newSize === 'tablet') && previousSize === 'mobile') {
          // Layout reorganizes first, then right panel slides in
          setTimeout(() => setRightPanelExpanded(true), motionTokens.panelTransition.stagger * 1000)
        }
      }
    }
  ...
  }, [viewportSize, setViewportSize, setSidebarExpanded, setRightPanelExpanded])
