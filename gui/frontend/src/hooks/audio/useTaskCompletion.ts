...
export const useTaskCompletion = (
  ...
  useEffect(() => {
    ...
    const completedTask = tasks.find(task => 
      task.task_id === pendingTaskId && task.status === 'completed'
    )
    ...
    const displayText = completedTask.request?.text || 
                       completedTask.result?.text_preview || 
                       'Generated audio'
    const provider = completedTask.request?.provider || completedTask.result?.provider
    const voiceId = completedTask.request?.sts_id || completedTask.result?.voice_id
    
    // Trigger audio playback through event system
    playGeneratedAudio({
      url: audioUrls[0], // Use first audio file for single player
      primaryText: displayText.length > 50 
...
export const useAnyTaskCompletion = (
  ...
  useEffect(() => {
    ...
    const newlyCompletedTasks = tasks.filter(task => 
      task.status === 'completed' && !processedTaskIds.current.has(task.task_id)
    )
