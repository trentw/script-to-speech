  ...
  describe('Task Polling', () => {
    it('should poll task status at 1 second intervals', async () => {
      ...
      mockApiService.getTaskStatus.mockImplementation(() => {
        callCount++
        if (callCount === 1) return Promise.resolve({ data: pendingTask, error: null })
        if (callCount === 2) return Promise.resolve({ data: processingTask, error: null })
        if (callCount === 3) return Promise.resolve({ data: completedTask, error: null })
    ...
    it('should stop polling when task fails', async () => {
      ...
      mockApiService.getTaskStatus.mockImplementation(() => {
        callCount++
        if (callCount === 1) return Promise.resolve({ data: processingTask, error: null })
    ...
    it('should replace existing task with same ID', async () => {
      ...
      mockApiService.getTaskStatus.mockResolvedValueOnce({ data: updatedTask, error: null })
