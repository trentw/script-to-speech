...
describe('useAudio Hook', () => {
  ...
  describe('Loading Audio', () => {
    it('should load audio from URL', async () => {
      // Act
      const { result } = renderHook(() => useAudio({ src: 'https://example.com/audio.mp3' }))
    ...
    it('should validate URL protocol', () => {
      ...
      // Assert
      expect(result.current.error).toBe('Invalid audio URL: only HTTP/HTTPS protocols are allowed')
      expect(result.current.isReady).toBe(false)
    ...
    it('should clear state when loading new audio', async () => {
      ...
      const { result } = renderHook(() => useAudio({ src: 'https://example.com/audio1.mp3' }))
  ...
  describe('Cleanup', () => {
    it('should remove event listeners on unmount', async () => {
      ...
      // Act
      const { unmount } = renderHook(() => useAudio({ src: 'https://example.com/audio.mp3' }))
