...
import { server as _server } from '@/test/setup';
...
vi.mock('../../services/api', () => ({
  apiService: {
    getProviderVoices: (provider: string) =>
      mockApiService.getProviderVoices(provider),
  },
}));
...
describe('useVoiceLibrary Hook', () => {
  ...
  describe('Successful Data Fetching', () => {
    it('should load voice library when provider is set and connected', async () => {
      ...
      await waitFor(() => {
        expect(mockApiService.getProviderVoices).toHaveBeenCalledWith(
          'elevenlabs'
        );
      });
    ...
    it('should not fetch when provider is not provided', async () => {
      // Act
      const { result } = renderHook(() =>
        useVoiceLibrary('connected', undefined)
      );
  ...
  describe('Provider Changes', () => {
    it('should fetch new provider when provider changes', async () => {
      ...
      await waitFor(() => {
        expect(result.current.voiceLibrary.elevenlabs).toEqual(
          elevenLabsVoices
        );
      });

      expect(mockApiService.getProviderVoices).toHaveBeenCalledTimes(2);
      expect(mockApiService.getProviderVoices).toHaveBeenNthCalledWith(
        1,
        'openai'
      );
      expect(mockApiService.getProviderVoices).toHaveBeenNthCalledWith(
