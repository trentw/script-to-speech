...
import { AlertCircle,ChevronRight, FileJson, Loader2, Upload } from 'lucide-react';
...
function VoiceCastingUpload() {
  ...
  const recentScreenplays = screenplays?.map(screenplay => ({
  ...
  const { data: sessionData } = useQuery({
    ...
    queryFn: async () => {
      ...
      const response = await apiService.getVoiceCastingSession(pendingSessionId);
      if (response.error) {
        throw new Error(response.error);
      }
      return response.data;
        ...
        {isLoading ? (
          ...
          <Card>
            <CardContent className="text-center py-8 text-muted-foreground">
              ...
              <p className="text-sm mt-2">Parse a screenplay PDF first to begin voice casting.</p>
            </CardContent>
          </Card>
        ) : (
          <div className="grid gap-4">
            ...
            <Card 
              ...
              onClick={() => !creatingSession && handleSelectRecent(screenplay.id)}
            >
              <CardContent className="flex items-center justify-between p-4">
                <div className="space-y-1">
                  <h3 className="font-medium">{screenplay.name}</h3>
                  <p className="text-sm text-muted-foreground">
                    {screenplay.characters} characters • {screenplay.lines} lines • {screenplay.date}
                  </p>
                </div>
